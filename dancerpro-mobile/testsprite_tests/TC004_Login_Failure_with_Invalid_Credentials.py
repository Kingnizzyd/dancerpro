...
async def run_test():
    ...
    try:
        ...
        # Enter registered email in email input and click Continue
        ...
        # Assert error message explaining invalid credentials is shown
        frame = context.pages[-1]
        ...
        # Assert user remains on login screen by checking presence of login screen title or welcome message
        title_locator = frame.locator('text=LoginScreen')
        welcome_locator = frame.locator('text=Welcome Back')
        assert await (title_locator.is_visible() or welcome_locator.is_visible()), 'User is not on the login screen after failed login attempt'
