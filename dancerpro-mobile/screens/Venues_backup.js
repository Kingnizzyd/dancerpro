...
export default function Venues() {
  ...
  function ShiftCard({ item }) {
    ...
    const venue = items.find(v => v.id === item.venueId);
    const client = clientOptions.find(c => c.id === item.clientId);
        ...
        <GradientCard variant="glow" style={styles.transactionModal}>
          ...
          <View style={styles.modalForm}>
            <View style={styles.clientSection}>
              ...
              <GradientButton
                title={shiftVenueId ? `🏢 ${items.find(v => v.id === shiftVenueId)?.name || 'Unknown'}` : '🏢 Select Venue'}
                onPress={() => {
            ...
            {clientOptions.length > 0 && (
              <View style={styles.clientSection}>
                <Text style={styles.clientLabel}>Client (Optional)</Text>
                <GradientButton
                  title={shiftClientId ? `👤 ${clientOptions.find(c => c.id === shiftClientId)?.name}` : '👤 No Client'}
                  onPress={() => {
